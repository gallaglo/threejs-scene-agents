...
def set_validation_result(
    ...
    print(f"[validator_agent] Scored code: {score}/100 at iteration {iteration}.", flush=True)
    if score < 80 and iteration < 3:
        print(f"[validator_agent] Score {score} is below threshold (80). Routing to refinement.", flush=True)
        tool_context.actions.route = "continue"
        return {
            "status": "continue",
            "message": f"Score {score} below threshold at iteration {iteration}. Proceeding to refinement.",
        }
    print(f"[validator_agent] Terminating loop (meets threshold or reached iteration limit). Final score: {score}.", flush=True)
    return {
        "status": "done",
